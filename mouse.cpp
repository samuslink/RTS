...
void MOUSE::Update(TERRAIN *terrain)
{
	...
	//Update pointer
	x += (int)(m_mouseState.lX * m_fSpeed);
	y += (int)(m_mouseState.lY * m_fSpeed);
...
{	
	...
	if(m_pMouseTexture != NULL)
	{
		...
		m_pSprite->Draw(m_pMouseTexture, &src[m_iType], NULL, &D3DXVECTOR3((float)x, (float)y, 0.0f),	0xffffffff);
		m_pSprite->End();
	}	
}
